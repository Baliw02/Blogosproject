...
<Sidebar v-if="screenW < 1200 "/>
    <carousel :perPage="1" :centerMode="false" :autoplay="true" :loop="true" :autoplayTimeout="15000" class="carousel" :paginationEnabled="true" :paginationPadding="2">
        <slide v-for="carousel_post in checkCarousel" :key="carousel_post.title">
                <a target="_blank" :href="carousel_post.link"><img class="carousel-image" :src="carousel_post.image"></a>
...
<div v-if="checkSpecial.length >= 1" class="special-slider">
    <button class="carousel-slider-left" v-if="!(carouselIndex == 0)" @click="carouselIndex -= 1"><i class="fas fa fa-angle-left"></i></button>
    <button class="carousel-slider-right" v-if="!(carouselIndex >= (checkSpecial.length / 3))" @click="carouselIndex += 1 "><i class="fas fa fa-angle-right"></i></button>
    <div class="slider-wrapper">
    <carousel :mouseDrag="false" :touchDrag="false" :scrollPerPage="false" :perPage="3" :paginationEnabled="false" :navigationEnabled="false" :navigationClickTargetSize="8" :navigateTo="carouselIndex">
    <slide v-for="(specialpost, i) in checkSpecial" :key="specialpost.title + specialpost.content + specialpost.uploader + specialpost.image">
    <div class="post-boxes">
        ...
        <a target="_blank" :href="specialpost.link"><img v-bind:href="specialpost.link" class="post-boxes-img" v-if="specialpost.image" :src="specialpost.image"></a>
        ...
        <div class="likes">
            <i @click="liking(specialpost, i)" class="fa fa-thumbs-up" aria-hidden="true"></i>
            {{specialpost.likes}}
        </div> 
        </div>
    </slide>   
...
<div v-if="items.length >= 1" class="outter-wrapper">
    <input class="search-in-post" type="text" placeholder="Search Post" v-model="search">
    <h1>Posts</h1>
    ...
    <br>
<div v-if="items.length >= 1" class="wrapper">
    <div v-for="(post, i) in searchFilter" :key="post.image + post.content + post._id + post.title + post.date + post.link + post.comments" class="post-boxes">
        ...
        <div class="likes">
            <i @click="liking(post, i)" class="fa fa-thumbs-up" aria-hidden="true"></i>
            {{post.likes}}
        </div>
    ...
    <img v-for="img in stock" :key="img.image" :src="img.image" alt="">
...
export default {
    ...
    methods:{
        ...
        async postComment(post, index){
            ...
            axios.post('http://localhost:8082/posts/' + post._id + '/comments', {comments: comment,}, {headers:headers}).then((res) => {
                if(res.status == 200){
                    this.items[index].comments.push({profile:this.username, message: this.comment})
                    this.comment = null
                }
            })
        },
        async removeComment(post, index, postIndex){
            ...
            axios.delete('http://localhost:8082/posts/' + post._id + '/comments', {headers:headers}).then((res) => {
                ...
                if(res.status == 200){
                    this.items[postIndex].comments.splice(index, 1)
                }
            })
        },
        async logOut(){
        ...
        async liking(post, index){
            ...
            axios.post('http://localhost:8082/posts/' + post._id + '/likes', {likedBy: this.username}, {headers: headers}).then((response) => {
                this.items.likes = response.data.likes
                ...
                // if(response.status.title == "liked"){
                //     console.log("-like")
                // }
        ...
        async getPostUrl(post){
            ...
            axios.get('http://localhost:8082/posts/' + post._id, {headers:headers})
